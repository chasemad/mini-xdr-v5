        ...
        if ping -c 1 -W 500 "$ip" > /dev/null 2>&1; then
            ...
            for port in 22 22022 2222; do
                if nc -z -w 1 "$ip" "$port" 2>/dev/null; then
                    ...
                    if timeout 5 ssh -p $port -i ~/.ssh/xdrops_id_ed25519 -o StrictHostKeyChecking=yes -o UserKnownHostsFile=~/.ssh/known_hosts -o ConnectTimeout=3 xdrops@$ip 'echo "XDR_TEST"' 2>/dev/null | grep -q "XDR_TEST"; then
