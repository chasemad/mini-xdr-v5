...
- **Infrastructure**: Local orchestration via `scripts/start-all.sh`; legacy AWS/Azure manifests are
  archived, and Kubernetes manifests in `k8s/` need validation before use.
...
- **Configuration**: Environment-driven settings via `.env`/`.env.local` with HMAC/JWT support.
...
- **Deployment validation**: Prioritize Docker Compose and local T-Pot; validate Kubernetes configs
  against the current codebase before production. AWS materials are archived.
...
2. **CI/CD Pipeline**: Decide whether any cloud CI/CD deployment remains part of the workflow; if not,
   remove or archive `buildspec-*.yml` and related documentation.
