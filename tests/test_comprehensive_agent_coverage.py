...
ATTACK_SCENARIOS = {
    ...
    "reconnaissance": {
        ...
        "expected_investigations": [
            ...
            "scanning_pattern_investigation",
        ],
    },
    ...
    "password_spray": {
        ...
        "expected_investigations": [
            "spray_pattern_analysis",
            "distributed_attack_investigation",
        ],
    },
    # 12. Insider Threat
    "insider_threat": {
    ...
    for cmd_idx, command in enumerate(scenario["chat_commands"]):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                # Regular response
                if not data.get("workflow_created") and not data.get(
                    "investigation_started"
                ):
                    print_info(
                        f"    Response type: {data.get('analysis_type', 'contextual_chat')}"
                    )
            ...
            results["commands_tested"].append({"command": command, "error": str(e)})

    # Determine success
    results["success"] = (
        len(results["workflows_created"]) > 0
        or len(results["investigations_started"]) > 0
    )

    if results["success"]:
        print_success(f"\n✅ Scenario '{scenario_name}' PASSED")
        print_info(
            f"   Workflows: {len(results['workflows_created'])}, Investigations: {len(results['investigations_started'])}"
        )
    else:
        print_error(
...
async def run_comprehensive_tests():
    ...
    async with httpx.AsyncClient(
        timeout=60.0, headers={"x-api-key": API_KEY, "Content-Type": "application/json"}
    ) as client:
        ...
        for idx, (scenario_name, scenario) in enumerate(ATTACK_SCENARIOS.items()):
            ...
            result = await test_attack_scenario(
                client, scenario_name, scenario, incident_id
            )
        ...
        total_investigations = sum(
            len(r["investigations_started"]) for r in all_results
        )
        ...
        for result in all_results:
            ...
            print(
                f"  {status} - {result['attack_type']}: {workflows}W / {investigations}I"
            )
        ...
        agents_coverage = (
            (len(agents_triggered) / len(expected_agents) * 100)
            if expected_agents
            else 0
        )

        print(
            f"  Agent Coverage: {len(agents_triggered)}/{len(expected_agents)} agents"
        )
