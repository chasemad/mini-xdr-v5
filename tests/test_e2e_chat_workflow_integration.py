...
async def test_workflow_creation_from_chat(
    client: httpx.AsyncClient, incident_id: int
) -> Dict[str, Any]:
        ...
        try:
            response = await client.post(
                ...
                json={
                    ...
                    "context": {"test": True, "source": "e2e_test"},
                },
            )
            ...
            if response.status_code == 200:
                ...
                if data.get("workflow_created"):
                    ...
                    print_info(
                        f"   - Approval required: {data.get('approval_required', False)}"
                    )
                    ...
                    print_info(
                        f"ℹ️  No workflow created (might not have recognized action keywords)"
                    )
            ...
            results.append(
                {"test": test["description"], "success": False, "reason": str(e)}
            )
...
async def test_investigation_trigger(
    client: httpx.AsyncClient, incident_id: int
) -> Dict[str, Any]:
    ...
    for query in test_queries:
        ...
        try:
            ...
            print_error(f"Exception: {e}")
            results.append({"query": query[:50], "success": False, "reason": str(e)})
...
async def test_workflow_sync(
    ...
    try:
        ...
        print_error(f"Exception: {e}")
        return {"test_name": "Workflow Sync", "passed": False, "reason": str(e)}
        ...
        try:
            response = await client.post(
                ...
                json={
                    ...
                    "incident_id": incident_id,
                    "context": {"attack_type": scenario["type"], "test": True},
                },
            )
            ...
            print_error(f"Exception: {e}")
            results.append(
                {"attack_type": scenario["type"], "success": False, "error": str(e)}
            )
...
async def run_all_tests():
    ...
    async with httpx.AsyncClient(
        timeout=30.0, headers={"x-api-key": API_KEY, "Content-Type": "application/json"}
    ) as client:
