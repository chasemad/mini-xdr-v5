...
test_honeypot() {
    ...
    if timeout 5 ssh -o ConnectTimeout=3 -o StrictHostKeyChecking=yes -o UserKnownHostsFile=~/.ssh/known_hosts admin@$PUBLIC_IP exit 2>/dev/null; then
    ...
    if timeout 10 ssh -i ~/.ssh/$KEY_NAME.pem -p 22022 -o ConnectTimeout=5 -o StrictHostKeyChecking=yes -o UserKnownHostsFile=~/.ssh/known_hosts ubuntu@$PUBLIC_IP "sudo /opt/mini-xdr/honeypot-status.sh" 2>/dev/null; then
