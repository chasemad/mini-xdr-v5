...
from .models import Incident, Event, ContainmentPolicy
...
class EnhancedContainmentEngine:
    ...
    async def evaluate_containment(
        ...
        if self.threat_intel:
            try:
                ...
                threat_intel_risk = intel_result.get("risk_score", 0.0)
                threat_category = intel_result.get("category", "unknown")
                risk_factors["threat_intel"] = threat_intel_risk
                reasoning.append(f"Threat intel risk: {threat_intel_risk:.2f} ({threat_category})")
            except Exception as e:
    ...
    async def _analyze_behavioral_patterns(self, recent_events: List[Event]) -> float:
        ...
        if len(recent_events) > 1:
            duration_hours = (recent_events[0].ts - recent_events[-1].ts).total_seconds() / 3600
            if duration_hours > 2:
