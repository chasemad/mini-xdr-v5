...
class Organization(Base):
    ...
    )  # profile|network_scan|agents|integrations|validation
    onboarding_data = Column(
        JSON, nullable=True
    )  # Wizard state (network ranges, scan results, tokens)
    onboarding_completed_at = Column(DateTime(timezone=True), nullable=True)
    first_login_completed = Column(Boolean, default=False)

    # Seamless onboarding (v2) settings
    onboarding_flow_version = Column(String(20), default="seamless")  # seamless|legacy
    auto_discovery_enabled = Column(Boolean, default=True)
    integration_settings = Column(
        JSON, nullable=True
    )  # Cloud integration settings including agent_public_base_url
...
class User(Base):
    ...
    role = Column(
        String(50), default="analyst", index=True
    )  # admin|analyst|viewer|soc_lead
    created_at = Column(DateTime(timezone=True), server_default=func.now())
    updated_at = Column(
        DateTime(timezone=True), server_default=func.now(), onupdate=func.now()
    )
...
class Event(Base):
    ...
    source_type = Column(
        String(32), default="cowrie", index=True
    )  # cowrie, suricata, osquery, etc.
    ...
    __table_args__ = (
        Index("ix_events_src_ts", "src_ip", "ts"),
        Index("ix_events_org_ts", "organization_id", "ts"),
    )
...
class Incident(Base):
    ...
    triage_note = Column(
        JSON, nullable=True
    )  # {summary, severity, recommendation, rationale}
    ...
    )  # brute_force|password_spray|credential_stuffing|etc.
    containment_confidence = Column(
        Float, default=0.0
    )  # Agent confidence in containment decision
    containment_method = Column(
        String(32), nullable=True
    )  # rule_based|ml_driven|ai_agent
    ...
    policy_id = Column(
        String(64), nullable=True
    )  # Reference to applied containment policy
    ...
    ai_analysis_timestamp = Column(
        DateTime(timezone=True), nullable=True
    )  # When analysis was done
    last_event_count = Column(
        Integer, default=0
    )  # Track new events to trigger re-analysis
...
class Action(Base):
    ...
    )  # Link to rollback action if any
...
class LogSource(Base):
    ...
    )  # cowrie, suricata, osquery, syslog, etc.
...
class MLModel(Base):
    ...
    organization_id = Column(
        ...
        nullable=True,
        index=True,
    )  # Nullable for shared/global models
    created_at = Column(DateTime(timezone=True), server_default=func.now())
    updated_at = Column(
        DateTime(timezone=True), server_default=func.now(), onupdate=func.now()
    )
    ...
    )  # isolation_forest, lstm_autoencoder, xgboost_classifier
...
class ContainmentPolicy(Base):
    ...
    # Agent settings
    agent_override = Column(
        Boolean, default=True
    )  # Allow agent to override this policy
...
class ResponseWorkflow(Base):
    ...
    impact_metrics = relationship(
        "ResponseImpactMetrics", backref="workflow", cascade="all, delete-orphan"
    )
...
class AdvancedResponseAction(Base):
    ...
    workflow_id = Column(
        Integer, ForeignKey("response_workflows.id"), nullable=True, index=True
    )
    incident_id = Column(Integer, ForeignKey("incidents.id"), index=True)
    parent_action_id = Column(
        Integer, ForeignKey("advanced_response_actions.id"), nullable=True
    )

    # Action definition
    action_type = Column(
        String(64), index=True
    )  # block_ip, isolate_host, deploy_firewall, etc.
    action_category = Column(
        String(32), index=True
    )  # network, endpoint, email, cloud, etc.
...
class ResponseApproval(Base):
    ...
    action_id = Column(
        Integer, ForeignKey("advanced_response_actions.id"), nullable=True, index=True
    )
...
class WorkflowTrigger(Base):
    ...
    )  # response|investigation|automation|reporting|qa
...
class AgentEnrollment(Base):
    ...
    )  # Agent's self-reported ID after registration
    ...
    enrollment_source = Column(
        String(64), nullable=True
    )  # onboarding_wizard|manual|api
    revoked_at = Column(DateTime(timezone=True), nullable=True)
    revoked_reason = Column(String(255), nullable=True)

    # Link to discovered asset if applicable
    discovered_asset_id = Column(
        Integer, ForeignKey("discovered_assets.id"), nullable=True
    )
    ...
    __table_args__ = (
        Index("ix_agent_enrollments_org_status", "organization_id", "status"),
    )


# =============================================================================
# SEAMLESS ONBOARDING V2 - CLOUD INTEGRATION MODELS
# =============================================================================


class IntegrationCredentials(Base):
    """Encrypted cloud provider integration credentials"""
    ...
    # Encrypted credential data (stored using Secrets Manager or KMS)
    credential_data = Column(JSON, nullable=False)  # Encrypted credential payload
...
class CloudAsset(Base):
    """Discovered cloud assets from auto-discovery"""
    ...
    )  # Cloud provider's unique asset ID
    region = Column(String(50), nullable=True)  # Cloud region/zone

    # Asset details
    asset_data = Column(
        JSON, nullable=False
    )  # Full asset metadata (instance type, IPs, security groups, etc.)
    tags = Column(JSON, nullable=True)  # Cloud provider tags
    ...
    agent_deployed = Column(Boolean, default=False, index=True)
    agent_status = Column(
        String(20), default="pending", index=True
    )  # pending, deploying, deployed, failed, incompatible
    agent_enrollment_id = Column(
        Integer, ForeignKey("agent_enrollments.id"), nullable=True
    )
    deployment_method = Column(
        String(50), nullable=True
    )  # ssm, ssh, winrm, cloud_init, etc.
    deployment_error = Column(Text, nullable=True)  # Error details if deployment failed

    # Relationships
    organization = relationship("Organization")
    agent_enrollment = relationship("AgentEnrollment")
