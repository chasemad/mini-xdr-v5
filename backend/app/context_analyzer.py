...
class ContextAnalyzer:
        ...
        return {
            ...
            "apt": {
                ...
                "mitre_tactics": [
                    ...
                    "TA0008",
                    "TA0010",
                ],  # Initial Access, Defense Evasion, Lateral Movement, Exfiltration
    ...
    async def analyze_comprehensive_context(
        ...
        try:
            ...
            context_analysis = {
                ...
                "threat_context": results[0]
                if not isinstance(results[0], Exception)
                else {},
                "response_context": results[1]
                if not isinstance(results[1], Exception)
                else {},
                "temporal_analysis": results[2]
                if not isinstance(results[2], Exception)
                else {},
                "behavioral_analysis": results[3]
                if not isinstance(results[3], Exception)
                else {},
                "infrastructure_analysis": results[4]
                if not isinstance(results[4], Exception)
                else {},
                "attribution_analysis": results[5]
            ...
            if include_predictions:
                context_analysis[
                    "predictive_analysis"
                ] = await self._generate_predictive_analysis(
                    incident, context_analysis, db_session
                )

            return {"success": True, "context_analysis": context_analysis}
    ...
    async def _analyze_response_context(
        self, incident: Incident, events: List[Event], db_session: AsyncSession
    ) -> ResponseContext:
    ...
    async def _calculate_threat_severity(
        self, incident: Incident, events: List[Event]
    ) -> float:
        ...
        severity_score += (
            complexity_score * self.severity_models["attack_complexity"]["weight"]
        )

        # Impact potential assessment
        impact_score = self._assess_impact_potential(incident, events)
        severity_score += (
            impact_score * self.severity_models["impact_potential"]["weight"]
        )

        # Threat intelligence integration
        intel_score = await self._get_threat_intel_severity(incident.src_ip)
        severity_score += (
            intel_score * self.severity_models["threat_intelligence"]["weight"]
        )
    ...
    def _build_attack_timeline(self, events: List[Event]) -> List[Dict[str, Any]]:
        ...
        for event in sorted_events:
            if event.ts:
                window = event.ts.replace(
                    minute=(event.ts.minute // 5) * 5, second=0, microsecond=0
                )
            ...
            timeline.append(
                {
                    ...
                    "unique_ports": len(
                        set(e.dst_port for e in window_events if e.dst_port)
                    ),
                    "event_types": list(
                        set(e.eventid for e in window_events if e.eventid)
                    ),
    ...
    def _assess_threat_impact(
        ...
        if any(
            port in [22, 3389, 445]
            for port in [e.dst_port for e in events if e.dst_port]
        ):
    ...
    async def _analyze_temporal_patterns(self, events: List[Event]) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_duration_seconds": (
                sorted_events[-1].ts - sorted_events[0].ts
            ).total_seconds(),
            "event_rate_per_minute": len(events)
            / max(1, (sorted_events[-1].ts - sorted_events[0].ts).total_seconds() / 60),
            "intervals_analysis": {
                "min": min(intervals),
                "max": max(intervals),
    ...
    async def _analyze_behavioral_indicators(
        ...
        return {
            ...
            "sophistication_indicators": self._extract_sophistication_indicators(
                behaviors
            ),
    ...
    async def _analyze_infrastructure_context(
        self, incident: Incident, events: List[Event]
    ) -> Dict[str, Any]:
        ...
        source_analysis = {
            ...
            "hosting_analysis": await self._analyze_hosting_infrastructure(
                incident.src_ip
            ),
    ...
    async def _analyze_attribution_indicators(
        self, incident: Incident, events: List[Event]
    ) -> Dict[str, Any]:
        ...
        attribution_indicators = {
            ...
            "campaign_indicators": await self._identify_campaign_indicators(
                incident, events
            ),
            "infrastructure_reuse": await self._analyze_infrastructure_reuse(
                incident.src_ip
            ),
        ...
        return {
            ...
            "campaign_likelihood": self._assess_campaign_likelihood(
                attribution_indicators
            ),
        }

    async def _get_similar_incidents(
        self, incident: Incident, db_session: AsyncSession
    ) -> List[Dict[str, Any]]:
                    ...
                    and_(
                        ...
                        or_(
                            ...
                            and_(
                                ...
                                getattr(Incident, "threat_category", None)
                                == getattr(incident, "threat_category", None),
                            ),
                        ),
    ...
    async def _generate_predictive_analysis(
        ...
        try:
            ...
            return {
                ...
                "recommended_monitoring": self._recommend_monitoring_enhancements(
                    context_analysis
                ),
    ...
    def _assess_impact_potential(
        ...
        if events:
            ...
            if (
                timestamps
                and (max(timestamps) - min(timestamps)).total_seconds() > 3600
            ):
                impact_score += 0.2
    ...
    def _assess_analysis_quality(self, analysis_results: List[Any]) -> Dict[str, Any]:
        ...
        quality_score = (
            successful_analyses / total_analyses if total_analyses > 0 else 0.0
        )
    ...
    async def _identify_technical_constraints(
        self, incident: Incident, db_session: AsyncSession
    ) -> Dict[str, Any]:
    ...
    async def _identify_campaign_indicators(
        self, incident: Incident, events: List[Event]
    ) -> Dict[str, Any]:
    ...
    def _calculate_similarity_score(
        self, incident1: Incident, incident2: Incident
    ) -> float:
        ...
        if (
            incident1.reason.lower() in incident2.reason.lower()
            or incident2.reason.lower() in incident1.reason.lower()
        ):
            score += 0.3
    ...
    def _analyze_exploitation_behavior(
        self, incident: Incident, events: List[Event]
    ) -> float:
    ...
    def _extract_sophistication_indicators(
    ...
    def _predict_lateral_movement_risk(
        self, threat_context: Dict[str, Any], infrastructure_analysis: Dict[str, Any]
    ) -> float:
        """Predict lateral movement risk"""
        ...
        if "ssh" in str(infrastructure_analysis) or "rdp" in str(
            infrastructure_analysis
        ):
            risk_score += 0.4
    ...
    def _predict_attack_duration(
        self, temporal_analysis: Dict[str, Any], threat_context: Dict[str, Any]
    ) -> float:
    ...
    def _recommend_monitoring_enhancements(
