...
class EffectivenessTracker:
    ...
    async def get_real_time_effectiveness(
        self, action_type: Optional[str] = None, timeframe_hours: int = 24
    ) -> Dict[str, Any]:
    ...
    async def get_effectiveness_trends(
        ...
        try:
            ...
            if action_types:
                actions_result = await db_session.execute(
                    select(AdvancedResponseAction).where(
                        and_(
                            AdvancedResponseAction.workflow_id.in_(
            ...
            trends = await self._calculate_effectiveness_trends_detailed(
                workflows, actions
            )
    ...
    async def _initialize_baseline_metrics(self, db_session: AsyncSession):
        ...
        try:
            ...
            self.logger.info(
                f"Initialized baseline metrics for {len(self.live_metrics)} action types"
            )
    ...
    async def _tracking_loop(self, db_session: AsyncSession):
        ...
        while self.tracking_active:
            try:
                ...
                await asyncio.sleep(
                    self.tracking_config["update_frequency_minutes"] * 60
                )
    ...
    async def _track_action_effectiveness(
        ...
        if action_type in self.live_metrics:
            ...
            new_success_rate = (1 - alpha) * metric.success_rate + alpha * (
    ...
    async def _track_workflow_effectiveness(
        ...
        successful_actions = len(
            [r for r in execution_results.get("results", []) if r.get("success")]
        )
        ...
        if playbook_name:
            await self._update_playbook_effectiveness(
                playbook_name, workflow_success_rate
            )
    ...
    async def _update_real_time_learning(
        ...
        slow_actions = [
            t for t in action_tracking if t.get("execution_time_ms", 0) > 600000
        ]  # > 10 min
        if slow_actions:
    ...
    async def _generate_effectiveness_insights(
        ...
        if learning_updates["optimization_opportunities"]:
            insights["optimization_insights"] = learning_updates[
                "optimization_opportunities"
            ]
    ...
    async def _update_effectiveness_metrics(self, db_session: AsyncSession):
        ...
        for action_type, updates in action_updates.items():
            if action_type in self.live_metrics:
                ...
                successful_new = (
                    metric.sample_size * metric.success_rate + updates["successful"]
                )

                metric.success_rate = (
                    successful_new / total_new if total_new > 0 else 0.0
                )
                ...
                # Update trend
                recent_success_rate = (
                    updates["successful"] / updates["total"]
                    if updates["total"] > 0
                    else 0.0
                )
    ...
    async def _cleanup_old_tracking_data(self):
        ...
        for action_type in list(self.performance_history.keys()):
            ...
            recent_history = [
                p
                for p in history
                if p.last_execution and p.last_execution > cutoff_time
            ]
    ...
    async def _calculate_overall_effectiveness(self) -> Dict[str, Any]:
        ...
        overall_success_rate = (
            weighted_success / total_weight if total_weight > 0 else 0.0
        )

        # Calculate confidence
        min_confidence = min(
            metric.confidence_score for metric in self.live_metrics.values()
        )
        ...
        # Assess trends
        improving_count = len(
            [m for m in self.live_metrics.values() if m.trend == "improving"]
        )
        declining_count = len(
            [m for m in self.live_metrics.values() if m.trend == "declining"]
        )
        ...
        return {
            ...
            "total_sample_size": sum(
                metric.sample_size for metric in self.live_metrics.values()
            ),
        }

    # Helper methods
    def _calculate_trend_direction(
        self, metric: EffectivenessMetric, new_success: bool
    ) -> str:
        ...
        new_rate = (
            historical_rate * (1 - recent_impact)
            + (1.0 if new_success else 0.0) * recent_impact
        )
    ...
    def _update_trend_direction(
        self, metric: EffectivenessMetric, recent_success_rate: float
    ) -> str:
    ...
    def _calculate_confidence_interval(
        self, metric: EffectivenessMetric
    ) -> Tuple[float, float]:
        ...
        return (max(0.0, p - margin_error), min(1.0, p + margin_error))
    ...
    async def _update_playbook_effectiveness(
        self, playbook_name: str, success_rate: float
    ):
        ...
        self.logger.info(
            f"Playbook '{playbook_name}' executed with {success_rate:.1%} success rate"
        )

    async def _calculate_effectiveness_trends_detailed(
        self, workflows: List[ResponseWorkflow], actions: List[AdvancedResponseAction]
    ) -> Dict[str, Any]:
        ...
        for date_str, data in sorted(daily_metrics.items()):
            ...
            if date_workflows:
                daily_success_rates = [
                    w.success_rate for w in date_workflows if w.success_rate is not None
                ]
                daily_avg_success = (
                    statistics.mean(daily_success_rates) if daily_success_rates else 0.0
                )
        ...
        return {
            ...
            "data_quality": "high"
            if len(daily_effectiveness) >= 7
            else "medium"
            if len(daily_effectiveness) >= 3
            else "low",
        }

    async def _calculate_action_trends(
        self, actions: List[AdvancedResponseAction], action_types: Optional[List[str]]
    ) -> Dict[str, Any]:
        ...
        for action_type, type_actions in actions_by_type.items():
            if len(type_actions) >= 5:  # Minimum for trend analysis
                ...
                sorted_actions = sorted(
                    type_actions, key=lambda a: a.created_at or datetime.min
                )
    ...
    async def _generate_trend_predictions(
        self, trends: Dict[str, Any], action_trends: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        if trends.get("daily_effectiveness"):
            recent_trend = (
                trends["daily_effectiveness"][-7:]
                if len(trends["daily_effectiveness"]) >= 7
                else trends["daily_effectiveness"]
            )
            if recent_trend:
                avg_recent_success = statistics.mean(
                    [d["success_rate"] for d in recent_trend]
                )
    ...
    def _assess_trend_data_quality(
        ...
        return {
            ...
            "data_completeness": len(
                [w for w in workflows if w.success_rate is not None]
            )
            / len(workflows)
            if workflows
            else 0,
    ...
    def _analyze_daily_trends(self, daily_data: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        return {
            "linear_trend": trend,
            "volatility": statistics.stdev(success_rates)
            if len(success_rates) > 1
            else 0.0,
            "best_day": max(daily_data, key=lambda d: d["success_rate"])
            if daily_data
            else None,
            "worst_day": min(daily_data, key=lambda d: d["success_rate"])
            if daily_data
            else None,
        }
