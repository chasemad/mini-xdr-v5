...
try:
    ...
    from .model_versioning import ab_test_manager, model_registry, performance_monitor
    from .online_learning import adapt_models_with_new_data, online_learning_engine
...
class ContinuousLearningPipeline:
    ...
    async def start_learning_loop(self):
        ...
        logger.info(
            f"Started {len(self.tasks)} learning tasks (Phase 2B features: {ADVANCED_ML_AVAILABLE})"
        )
    ...
    async def _baseline_update_loop(self):
        ...
        while self.running:
            try:
                ...
                self.learning_metrics["last_baseline_update"] = datetime.now(
                    timezone.utc
                )
        ...
        while self.running:
            try:
                async with AsyncSessionLocal() as db:
                    ...
                    if success:
                        self.learning_metrics["last_model_retrain"] = datetime.now(
                            timezone.utc
                        )
                        self.learning_metrics["successful_updates"] += 1
    ...
    async def _pattern_update_loop(self):
        ...
        while self.running:
            try:
                ...
                self.learning_metrics["last_pattern_refresh"] = datetime.now(
                    timezone.utc
                )
                self.learning_metrics["successful_updates"] += 1
    ...
    async def _retrain_ml_models(self, db: AsyncSession) -> bool:
        ...
        try:
            ...
            logger.warning(
                f"ML model retraining failed (expected with empty database): {e}"
            )
            return False
    ...
    async def _get_training_events(self, db: AsyncSession) -> List[Event]:
        ...
        try:
            ...
            window_start = datetime.now(timezone.utc) - timedelta(
                days=self.training_history_days
            )
            ...
            incident_ips = {row[0] for row in incident_result if row and row[0]}
            ...
            events = result.scalars().all() if result else []

            logger.info(
                f"Retrieved {len(events)} clean training events, excluded {len(incident_ips)} incident IPs"
            )
            return events
        except Exception as e:
            logger.warning(f"Failed to get training events: {e}")
            return []
        ...
        try:
            ...
            for incident in recent_incidents:
                ...
                if (
                    ...
                    and hasattr(incident, "created_at")
                    and incident.created_at
                ):
                    time_to_dismiss = datetime.now(timezone.utc) - incident.created_at
                    if (
                        time_to_dismiss.total_seconds() < 3600
                    ):  # Dismissed within 1 hour
                        false_positives += 1
                    else:
                        true_positives += 1
            ...
            if total_incidents > 5:  # Need minimum sample size
                ...
                if false_positive_rate > 0.3:  # Too many false positives
                    behavioral_analyzer.adaptive_threshold = min(
                        0.8, behavioral_analyzer.adaptive_threshold + 0.05
                    )
                    logger.info(
                        f"Increased behavioral threshold to {behavioral_analyzer.adaptive_threshold:.2f} (FPR: {false_positive_rate:.2f})"
                    )
                elif (
                    false_positive_rate < 0.1
                ):  # Very few false positives, can be more aggressive
                    behavioral_analyzer.adaptive_threshold = max(
                        0.4, behavioral_analyzer.adaptive_threshold - 0.05
                    )
                    logger.info(
                        f"Decreased behavioral threshold to {behavioral_analyzer.adaptive_threshold:.2f} (FPR: {false_positive_rate:.2f})"
                    )
        except Exception as e:
            logger.warning(f"Pattern update error (expected with empty database): {e}")
    ...
    async def _adjust_detection_sensitivity(self, db: AsyncSession):
        ...
        try:
            ...
            if not recent_incidents:
                logger.debug("No recent incidents to adjust sensitivity")
                return
            ...
            logger.warning(
                f"Sensitivity adjustment error (expected with empty database): {e}"
            )
            ...
            try:
                async with AsyncSessionLocal() as db:
                    ...
                    if len(recent_events) >= self.min_online_adaptation_events:
                        ...
                        adaptation_result = await adapt_models_with_new_data(
                            recent_events
                        )

                        if adaptation_result.get("success"):
                            self.learning_metrics[
                                "last_online_adaptation"
                            ] = datetime.now(timezone.utc)
                            self.learning_metrics["online_adaptations"] += 1

                            logger.info(
                                f"Online adaptation completed: {adaptation_result.get('samples_processed')} samples"
                            )
                        else:
                            logger.warning(
                                f"Online adaptation failed: {adaptation_result.get('error', 'Unknown error')}"
                            )
                ...
                logger.warning(
                    f"Online adaptation loop error (expected with empty database): {e}"
                )
                await asyncio.sleep(60)  # Retry in 1 minute
    ...
    async def _ensemble_optimization_loop(self):
        ...
        while self.running:
            try:
                ...
                logger.warning(
                    f"Ensemble optimization loop error (expected with empty database): {e}"
                )
                await asyncio.sleep(3600)  # Retry in 1 hour
                ...
                if model_registry and performance_monitor:
                    ...
                    for model_version in production_models:
                        ...
                        if health_summary["status"] in ["unhealthy", "degraded"]:
                            ...
                            logger.warning(
                                f"Model performance alert: {model_version.model_id} v{model_version.version} is {health_summary['status']}"
                            )

                    # Clean old alerts (keep last 100)
                    if len(self.model_performance_alerts) > 100:
                        self.model_performance_alerts = self.model_performance_alerts[
                            -100:
                        ]

                self.learning_metrics["last_performance_check"] = datetime.now(
                    timezone.utc
                )
    ...
    async def _get_recent_events_for_adaptation(self, db: AsyncSession) -> List[Event]:
        ...
        try:
            ...
            logger.warning(f"Failed to get recent events for adaptation: {e}")
            return []
    ...
    async def _optimize_ensemble_models(self, training_events: List[Event]) -> bool:
        ...
        try:
            ...
            if (
                not training_data
                or "X" not in training_data
                or "y" not in training_data
            ):
            ...
            if model_registry:
                ...
                logger.info(
                    f"Registered optimized ensemble: {model_version.model_id} v{model_version.version}"
                )
    ...
    async def create_ab_test(
        ...
        try:
            test_id = ab_test_manager.create_ab_test(
                ...
                or f"Comparing {model_a_id} v{model_a_version} vs {model_b_id} v{model_b_version}",
    ...
    def get_enhanced_learning_status(self) -> Dict[str, Any]:
        ...
        if ADVANCED_ML_AVAILABLE:
            ...
            base_status.update(
                {
                    "phase_2b_features": {
                        ...
                        "online_adaptations": self.learning_metrics[
                            "online_adaptations"
                        ],
                        "ensemble_optimizations": self.learning_metrics[
                            "ensemble_optimizations"
                        ],
                        "drift_detections": self.learning_metrics["drift_detections"],
                        "last_online_adaptation": self.learning_metrics[
                            "last_online_adaptation"
                        ],
                        "last_ensemble_optimization": self.learning_metrics[
                            "last_ensemble_optimization"
                        ],
                        "last_performance_check": self.learning_metrics[
            ...
            if online_learning_engine:
                ...
                ] = online_learning_engine.get_drift_status()

        return base_status

    async def explain_recent_prediction(
        self, incident_id: int, context: Optional[Dict[str, Any]] = None
        ...
        try:
            async with AsyncSessionLocal() as db:
                ...
                feature_data["unique_ports"] = len(
                    set(e.dst_port for e in events if e.dst_port)
                )
                feature_data["avg_message_length"] = np.mean(
                    [len(e.message or "") for e in events]
                )
                feature_data["anomaly_score"] = np.mean(
                    [e.anomaly_score or 0 for e in events]
                )
            ...
            logger.error(
                f"Failed to generate explanation for incident {incident_id}: {e}"
            )
            return None
