...
class EnsembleMLDetector:
    ...
    def __init__(self, 
        ...
        logger.info(f"   Windows specialist loaded: {self.windows_specialist is not None}")
    
    def _load_network_model(self):
        """Load existing network attack models"""
        try:
            general_model_path = self.network_model_dir / "general" / "threat_detector.pth"
            ...
            # (Same architecture as aws/train_local.py)
            ...
            if scaler_path.exists():
                ...
                self.network_scaler = joblib.load("/Users/chasemad/Desktop/mini-xdr/models/scaler.pkl")
    ...
    def _create_threat_detector_model(self, input_dim, hidden_dims, num_classes, dropout_rate, use_attention):
        ...
        class ThreatDetector(nn.Module):
            def __init__(self, input_dim, hidden_dims, num_classes, dropout_rate, use_attention):
                super().__init__()
        ...
        return ThreatDetector(input_dim, hidden_dims, num_classes, dropout_rate, use_attention)
    ...
    def _load_windows_specialist(self):
        ...
        try:
            ...
            from aws.train_windows_specialist_13class import Windows13ClassSpecialist
    ...
    def _load_legacy_windows_specialist(self):
        ...
        try:
            legacy_path = Path("/Users/chasemad/Desktop/mini-xdr/models/windows_specialist/windows_specialist.pth")
            if not legacy_path.exists():
                return None
            
            checkpoint = torch.load(legacy_path, map_location=self.device)
            from aws.train_windows_specialist import WindowsSpecialistModel
            model = WindowsSpecialistModel(input_dim=79, hidden_dims=[256, 128, 64], num_classes=7)
            model.load_state_dict(checkpoint['model_state_dict'])
            model.to(self.device)
            model.eval()
    ...
    async def detect_threat(self, event_features: np.ndarray) -> Dict:
        ...
        if results['network_prediction'] and results['windows_prediction']:
            ...
            if results['windows_prediction']['class'] > 0 and results['windows_prediction']['confidence'] > 0.7:
            ...
            elif results['network_prediction']['class'] > 0 and results['network_prediction']['confidence'] > 0.7:
