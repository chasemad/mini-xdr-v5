...
from .ai_response_advisor import (
...
from .response_optimizer import OptimizationStrategy, get_response_optimizer
...
class LearningResponseEngine:
    ...
    async def generate_adaptive_recommendations(
        ...
        try:
            ...
            if not optimized_result.get("success"):
                ...
                optimized_recommendations = optimized_result[
    ...
    async def _apply_adaptive_learning(
        ...
        learning_data = await self._get_relevant_learning_data(
            context_analysis, db_session
        )
        ...
        for rec in recommendations:
            ...
            adapted_rec = await self._adapt_recommendation(
                rec, learning_data, context_analysis
            )
            adapted_recommendations.append(adapted_rec)

        # Add learned actions if confidence is high
        learned_actions = await self._get_learned_actions(
            context_analysis, learning_data
        )
        for learned_action in learned_actions:
            if (
                learned_action["confidence"]
    ...
    async def _adapt_recommendation(
        ...
        historical_performance = learning_data.get("action_performance", {}).get(
            action_type, {}
        )
        ...
        parameter_learning = learning_data.get("parameter_optimization", {}).get(
            action_type, {}
        )
        ...
        timing_learning = learning_data.get("timing_optimization", {}).get(
            action_type, {}
        )
        if timing_learning:
    ...
    async def _get_learned_actions(
        self, context_analysis: Dict[str, Any], learning_data: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        ...
        category_learning = learning_data.get("category_patterns", {}).get(
            threat_category, {}
        )

        for action_type, learning_info in category_learning.get(
            "emergent_actions", {}
        ).items():
            if (
                learning_info.get("confidence", 0.0)
                > self.learning_config["confidence_threshold"]
            ):
    ...
    async def _generate_comprehensive_response_plan(
        ...
        primary_plan = {
            ...
            "estimated_duration": sum(
                r.get("estimated_duration", 300) for r in recommendations[:5]
            ),
            "confidence": sum(r.get("confidence", 0.0) for r in recommendations[:5])
            / min(len(recommendations), 5),
            "approval_required": any(
                r.get("approval_required", False) for r in recommendations[:5]
            ),
        }
        ...
        fallback_plan = {
            "strategy": "fallback",
            "actions": await self._create_fallback_actions(
        ...
        threat_context = context_analysis.get("threat_context", {})
        response_strategy = self._determine_execution_strategy(
            threat_context, primary_plan
        )
    ...
    async def _get_relevant_learning_data(
        self, context_analysis: Dict[str, Any], db_session: AsyncSession
    ) -> Dict[str, Any]:
        ...
        learning_data = {
            "action_performance": await self._analyze_action_performance(
                similar_workflows, db_session
            ),
            "parameter_optimization": await self._analyze_parameter_performance(
                similar_workflows
            ),
            "timing_optimization": await self._analyze_timing_patterns(
                similar_workflows
            ),
            ...
            "context_similarity": self._calculate_context_similarity(
                similar_workflows, threat_context
            ),
    ...
    async def _get_similar_historical_workflows(
        self, threat_category: str, severity_score: float, db_session: AsyncSession
        ...
        result = await db_session.execute(
            ...
            .where(
                and_(
                    ...
                    - timedelta(days=self.learning_config["max_learning_history_days"]),
        ...
        for workflow in workflows:
            ...
            for step in workflow.steps:
                ...
                if action_type and parameters:
                    ...
                    parameter_data[action_type]["successful_parameters"].append(
                        parameters
                    )
        ...
        for workflow in workflows:
            ...
            for i, step in enumerate(workflow.steps):
                ...
                if action_type and i > 0:
                    ...
                    timing_data[timing_key]["success_rates"].append(
            ...
            if workflow.success_rate and workflow.success_rate > 0.8 and workflow.steps:
                for step in workflow.steps:
                    ...
                    if action_type:
                        if (
                            action_type
                            not in category_patterns[category]["common_actions"]
                        ):
                            category_patterns[category]["common_actions"][
                                action_type
                            ] = 0
    ...
    def _calculate_context_similarity(
        self, workflows: List[ResponseWorkflow], current_context: Dict[str, Any]
    ) -> float:
    ...
    def _assess_sample_quality(
        ...
        recent_workflows = len(
            [
                ...
                for w in workflows
                if w.created_at
                and w.created_at > datetime.now(timezone.utc) - timedelta(days=30)
            ]
        )
        ...
        fallback_actions.append(
            {
                ...
                "confidence": 0.7,
                "priority": 2,
                "parameters": {"dump_type": "minimal", "encryption": True},
    ...
    async def _create_emergency_actions(
    ...
    def _determine_execution_strategy(
        self, threat_context: Dict[str, Any], primary_plan: Dict[str, Any]
    ) -> str:
    ...
    def _generate_execution_guidelines(
        ...
        if threat_context.get("severity_score", 0) > 0.8:
            guidelines.append(
                "Consider escalating to senior analyst due to high severity"
            )

        if any(rec.get("approval_required", False) for rec in recommendations):
            guidelines.append(
                "Obtain required approvals before executing high-impact actions"
            )
    ...
    async def _get_learning_insights(
        self, incident_id: int, db_session: AsyncSession
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            learning_event = {
                ...
                "learning_score": self._calculate_learning_score(
                    execution_results, analyst_feedback
                ),
            }
            ...
            model_updates = await self._update_learning_models(
                learning_event, db_session
            )
    ...
    def _extract_optimal_parameters(
    ...
    async def _get_recent_learning_data(
    ...
    async def _update_learning_models(
        self, learning_event: Dict[str, Any], db_session: AsyncSession
    ) -> Dict[str, Any]:
    ...
    def _analyze_learning_progress(self) -> Dict[str, Any]:
        ...
        recent_scores = [
            event["learning_score"] for event in self.learning_history[-10:]
        ]
    ...
    async def _generate_adaptation_recommendations(
        self, learning_event: Dict[str, Any]
    ) -> List[str]:
        ...
        if learning_event["learning_score"] > 0.8:
            recommendations.append(
                "Consider increasing automation level for similar incidents"
            )
        elif learning_event["learning_score"] < 0.5:
            recommendations.append(
                "Review and refine response strategies for this incident type"
            )
