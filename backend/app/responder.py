...
class ResponderAgent:
    ...
    async def test_connection(self) -> Tuple[str, str]:
        ...
        status, stdout, stderr = await self.execute_command(
    ...
    async def execute_command(
        ...
        try:
            ...
            result = await loop.run_in_executor(
                None, self._sync_execute_command, command, timeout
            )
            ...
            if result[0] == "success":
                ...
                logger.warning(
                    f"Paramiko failed: {result[2]}, trying subprocess SSH..."
                )
    ...
    async def _subprocess_ssh_command(
        ...
        try:
            ...
            loop = asyncio.get_event_loop()
            result = await loop.run_in_executor(
                None, self._run_subprocess, ssh_cmd, timeout
            )
            return result
        except Exception as e:
    ...
    def _run_subprocess(self, cmd: list, timeout: int) -> Tuple[str, str, str]:
        ...
        try:
            ...
            logger.info(
                f"Subprocess SSH result: status={status}, returncode={result.returncode}"
            )
...
async def block_ip(ip: str, duration_seconds: int = None) -> Tuple[str, str]:
    """
    Block an IP address using T-Pot connector or local SSH with optional temporary blocking
    ...
    if _is_private_ip(ip) and not settings.allow_private_ip_blocking:
        logger.warning(
            f"Refusing to block private IP: {ip} (allow_private_ip_blocking=False)"
        )
        return (
            "failed",
            f"Refusing to block private IP: {ip} - Enable allow_private_ip_blocking for testing",
        )

    # Try T-Pot connector first (if connected)
    try:
        ...
        if connector.is_connected:
            ...
            logger.debug(f"T-Pot block result: {result}")

            if result["success"]:
                status = "success"
                detail = (
                    f"IP {ip} blocked on T-Pot via UFW\n{result.get('message', '')}"
                )
                if result.get("note"):
                    detail += f"\nNote: {result['note']}"
            else:
                ...
                if result.get("note"):
                    detail += f"\nNote: {result['note']}"
                logger.error(f"T-Pot blocking failed for {ip}: {detail}")
        else:
            # Fallback to direct SSH (old method)
            logger.warning(
                f"T-Pot not connected (connector.is_connected={connector.is_connected}), using direct SSH fallback"
            )
            ufw_check_status, _, _ = await responder.execute_command(
                "which ufw", timeout=5
            )
        ...
        logger.error(f"T-Pot blocking exception for {ip}: {e}", exc_info=True)
        # Fallback to old SSH method
        logger.info("Attempting fallback to direct SSH method")
        try:
            ufw_check_status, _, _ = await responder.execute_command(
                "which ufw", timeout=5
            )
            ...
            status, stdout, stderr = await responder.execute_command(command)
            detail = f"Fallback Command: {command}\nStatus: {status}\nStdout: {stdout}\nStderr: {stderr}"
        except Exception as fallback_error:
            logger.error(
                f"Fallback SSH method also failed: {fallback_error}", exc_info=True
            )
            status = "failed"
            detail = f"All blocking methods failed. Original error: {str(e)}, Fallback error: {str(fallback_error)}"
    ...
    if status == "success":
        ...
        if duration_seconds:
            logger.info(
                f"Scheduling auto-unblock for {ip} in {duration_seconds} seconds"
            )
