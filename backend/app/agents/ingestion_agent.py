...
class IngestionAgent:
    ...
    async def start(self):
        ...
        self.logger.info(f"Starting ingestion agent for {self.config.source_type}:{self.config.hostname}")
        ...
        for source_name, log_path in self.config.log_paths.items():
            task = asyncio.create_task(
                self._collect_logs(source_name, log_path), 
    ...
    async def _parse_log_line(self, line: str, source_name: str) -> Optional[Dict[str, Any]]:
    ...
    def _parse_syslog_line(self, line: str) -> Dict[str, Any]:
        ...
        event = {
            'timestamp': ' '.join(parts[:3]) if len(parts) >= 3 else datetime.utcnow().isoformat(),
    ...
    async def _flush_buffer(self):
        ...
        self.logger.error(f"Failed to send {len(events_to_send)} events after {self.config.max_retries} attempts")
    ...
    async def _send_events(self, payload: Dict[str, Any]) -> bool:
        ...
        try:
            async with self.session.post(
                f"{self.config.backend_url}/ingest/multi",
                headers=headers,
                data=data
            ) as response:
    ...
    async def _periodic_stats(self):
        ...
        while self.running:
            ...
            rate = self.stats['events_collected'] / max(uptime, 1) * 60  # events per minute
