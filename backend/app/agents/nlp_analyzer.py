...
class ThreatQueryParser:
    ...
    def parse_query(self, query: str) -> QueryContext:
        ...
        confidence_score = self._calculate_confidence(entities, time_constraints, threat_categories)
    ...
    def _extract_time_constraints(self, query: str) -> Optional[Dict[str, Any]]:
        ...
        relative_pattern = r'\b(?:last|past)\s+(\d+)\s+(hour|hours|day|days|week|weeks|month|months)\b'
        ...
        # Look for absolute time expressions
            ...
            if 'today' in query.lower():
                time_info = {'type': 'relative', 'hours_back': 24, 'description': 'Today'}
            elif 'yesterday' in query.lower():
                time_info = {'type': 'relative', 'hours_back': 48, 'description': 'Yesterday'}
            elif 'this week' in query.lower():
                time_info = {'type': 'relative', 'hours_back': 168, 'description': 'This week'}
    ...
    def _calculate_confidence(self, entities: Dict, time_constraints: Optional[Dict], threat_categories: List[str]) -> float:
...
class NaturalLanguageThreatAnalyzer:
    ...
    def __init__(self):
        ...
        if LANGCHAIN_AVAILABLE:
            ...
            self.logger.warning("LangChain not available - using fallback NLP processing")
    
    def _initialize_llm(self):
        """Initialize LLM components"""
        try:
            # Check for OpenAI API key
            openai_key = getattr(settings, 'OPENAI_API_KEY', None) or getattr(settings, 'XAI_API_KEY', None)
            
            if openai_key:
                # Initialize ChatOpenAI with appropriate model
                self.llm = ChatOpenAI(
                    temperature=self.response_temperature,
                    model_name="gpt-4" if "sk-" in openai_key else "grok-beta",  # Use Grok for XAI keys
                    max_tokens=1000,
                ...
                self.logger.warning("No LLM API key found - using pattern-based analysis only")
    ...
    async def analyze_natural_language_query(
        ...
        try:
            ...
            structured_query = await self._generate_structured_query(query, query_context)
    ...
    async def _execute_analysis(
        ...
        if context.query_type == QueryType.INCIDENT_SEARCH:
            ...
            findings = await self._recognize_patterns(structured_query, incidents, events)
            ...
            findings = await self._analyze_attribution(structured_query, incidents, events)
        
        elif context.query_type == QueryType.RECOMMENDATION:
            findings = await self._generate_action_recommendations(structured_query, incidents, events)
        
        return findings
    
    async def _search_incidents(self, query: Dict[str, Any], incidents: List[Incident]) -> List[Dict[str, Any]]:
        ...
        if query.get("filters", {}).get("src_ips"):
            ...
            filtered_incidents = [inc for inc in filtered_incidents if inc.src_ip in target_ips]
        
        if query.get("filters", {}).get("time_range"):
            time_range = query["filters"]["time_range"]
            if time_range.get("hours_back"):
                cutoff_time = datetime.utcnow() - timedelta(hours=time_range["hours_back"])
                filtered_incidents = [inc for inc in filtered_incidents if inc.created_at >= cutoff_time]
    ...
    async def _analyze_threats(
        ...
        for threat_type, threat_incidents in threat_groups.items():
            if len(threat_incidents) >= 2:  # Only include patterns with multiple incidents
                findings.append({
                    ...
                    "severity_distribution": self._calculate_severity_distribution(threat_incidents),
                    "relevance_score": 0.8
                })
    ...
    async def _extract_iocs(
        ...
        for event in events:
            if hasattr(event, 'raw') and event.raw:
                raw_data = json.dumps(event.raw) if isinstance(event.raw, dict) else str(event.raw)
        ...
        for ip_prefix, prefix_incidents in geo_clusters.items():
            if len(prefix_incidents) >= 3:  # Minimum for attribution
                findings.append({
                    ...
                    "attack_types": list(set(inc.reason for inc in prefix_incidents if inc.reason)),
    ...
    async def _generate_action_recommendations(
        ...
        if open_incidents > 0:
            recommendations.append(f"Consider containing {open_incidents} open incidents")
    ...
    async def _generate_ai_insights(
        ...
        try:
            ...
            incident_summary = self._prepare_incident_summary(incidents[:10])  # Limit context
...
"""
    ...
    async def _generate_recommendations(
        ...
        if context.query_type == QueryType.INCIDENT_SEARCH:
            recommendations.append("Review identified incidents for containment opportunities")
            ...
            recommendations.append("Update detection rules based on threat characteristics")
        ...
        if context.time_constraints and context.time_constraints.get("hours_back", 0) <= 1:
            recommendations.append("Consider immediate containment for recent incidents")
    ...
    async def _generate_follow_up_questions(
        ...
        if any(f.get("type") == "pattern_repeat_attackers" for f in findings):
            follow_ups.append("Would you like detailed analysis of the repeat attackers?")
    ...
    def _calculate_severity_distribution(self, incidents: List[Incident]) -> Dict[str, int]:
    ...
    def _find_time_clusters(self, incidents: List[Incident]) -> List[Dict[str, Any]]:
        ...
        for i in range(1, len(sorted_incidents)):
            time_diff = (sorted_incidents[i].created_at - current_cluster[-1].created_at).total_seconds()
            ...
            if time_diff <= 3600:
                ...
                if len(current_cluster) >= 2:
                    clusters.append({
                        ...
                        "duration_minutes": (current_cluster[-1].created_at - current_cluster[0].created_at).total_seconds() / 60
                    })
    ...
    def _prepare_findings_summary(self, findings: List[Dict[str, Any]]) -> str:
        ...
        summary_lines = []
        for finding in findings:
            summary_lines.append(f"Type: {finding.get('type', 'unknown')} - {finding.get('description', 'No description')}")
