...
class AttributionAgent:
    ...
    async def analyze_attribution(
        ...
        try:
            self.logger.info(
                f"Starting attribution analysis for {len(incidents)} incidents, {len(events)} events"
            )
            ...
            campaign_analysis = await self._correlate_campaigns(
                incidents, ttp_analysis, infrastructure_analysis
            )
    ...
    async def analyze_ip_reputation(self, ip_address: str) -> Dict[str, Any]:
        ...
        try:
            ...
            reputation_data["threat_categories"] = list(
                set(
    ...
    async def _analyze_infrastructure(
        ...
        shared_hosting = await self._analyze_shared_hosting(
            infrastructure_data["ip_asn_mapping"]
        )
    ...
    async def _analyze_dns_relationships(self, ip: str) -> Optional[Dict[str, Any]]:
        ...
        dns_info = {"reverse_dns": None, "related_domains": []}
        ...
        return (
            dns_info if dns_info["reverse_dns"] or dns_info["related_domains"] else None
        )

    async def _identify_infrastructure_clusters(
        ...
        for asn, ips in asn_groups.items():
            if len(ips) >= 2 and asn != "unknown":
                cluster = {
                    ...
                    "shared_attributes": {
                        "asn": asn,
                        "organization": infrastructure_data["ip_asn_mapping"][
                            ips[0]
                        ].get("organization", ""),
                    },
        ...
        for country, ips in geo_groups.items():
            if len(ips) >= 3 and country != "unknown":
                cluster = {
                    ...
                    "ips": ips,
                    "shared_attributes": {"country": country},
    ...
    async def _analyze_shared_hosting(
        self, asn_mapping: Dict[str, Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
            ...
            for ttp_id, confidence in event_ttps.items():
                if ttp_id not in ttp_analysis["identified_ttps"]:
                    ...
                    ttp_analysis["identified_ttps"][ttp_id]["confidence"] = max(
                        ttp_analysis["identified_ttps"][ttp_id]["confidence"],
                        confidence,
                    )
    ...
    async def _analyze_behavioral_patterns(
        self, identified_ttps: Dict[str, Any], ttp_timeline: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        ...
        patterns["sophistication_score"] = min(
            total_sophistication / len(identified_ttps) if identified_ttps else 0.0, 1.0
        )
    ...
    async def _analyze_temporal_patterns(
        ...
        if len(all_timestamps) >= 2:
            ...
            temporal_analysis["activity_intensity"] = {
                ...
                "peak_day": max(daily_activity.items(), key=lambda x: x[1])[0]
                if daily_activity
                else None,
            }
            ...
            business_hours = sum(
                count for hour, count in hourly_activity.items() if 9 <= int(hour) <= 17
            )
    ...
    async def _correlate_campaigns(
        ...
        for src_ip, grouped_incidents in ip_groups.items():
            if len(grouped_incidents) >= 2:
                ...
                # Time span factor
                time_span = (
                    grouped_incidents[-1].created_at - grouped_incidents[0].created_at
                ).total_seconds() / 3600
                ...
                for cluster in infrastructure_analysis.get(
                    "infrastructure_clusters", []
                ):
        ...
        for ttp_signature, grouped_incidents in ttp_signatures.items():
            if len(grouped_incidents) >= 2:
                ...
                sophistication = ttp_analysis.get("behavioral_patterns", {}).get(
                    "sophistication_score", 0.0
                )
    ...
    async def _match_ttp_patterns(
        self, signature: Dict[str, Any], ttp_analysis: Dict[str, Any]
    ) -> float:
    ...
    async def _ai_attribution_analysis(
        ...
        context = {
            ...
            "infrastructure_clusters": len(
                infrastructure_analysis.get("infrastructure_clusters", [])
            ),
            "behavioral_sophistication": ttp_analysis.get(
                "behavioral_patterns", {}
            ).get("sophistication_score", 0.0),
        }
    ...
    def _calculate_overall_confidence(
        ...
        if campaign_analysis.get("potential_campaigns"):
            max_campaign_confidence = max(
                campaign["confidence"]
                for campaign in campaign_analysis["potential_campaigns"]
            )
    ...
    async def _update_attribution_tracking(self, attribution_results: Dict[str, Any]):
        ...
        for actor in attribution_results.get("actor_attribution", {}).get(
            "attributed_actors", []
        ):
            ...
            if actor_id not in self.threat_actors:
                ...
                self.threat_actors[actor_id].confidence_score = max(
                    self.threat_actors[actor_id].confidence_score, actor["confidence"]
                )

        # Update campaign tracking
        for campaign in attribution_results.get("campaign_analysis", {}).get(
            "potential_campaigns", []
        ):
        ...
        self.logger.info(
            f"Updated attribution tracking: {len(self.threat_actors)} actors, {len(self.campaigns)} campaigns"
        )
...
attribution_tracker = AttributionAgent()
