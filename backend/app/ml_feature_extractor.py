...
class MLFeatureExtractor:
    """Extract 79 features from events for ML inference"""
    ...
    def _get_feature_names(self) -> List[str]:
        ...
        return [
            ...
            'hour', 'day_of_week', 'is_weekend', 'is_business_hours', 'time_since_first_event',
            
            # Volume features (10)
            'event_count_1min', 'event_count_5min', 'event_count_1h', 'event_count_24h',
            'events_per_minute', 'events_per_hour', 'burst_score', 'sustained_activity',
            'connection_rate', 'packet_rate',
            
            # Port features (8)
            'dst_port_normalized', 'src_port_normalized', 'unique_dst_ports', 'unique_src_ports',
            'port_diversity', 'is_common_port', 'is_high_port', 'port_scan_score',
            
            # Protocol features (5)
            'is_tcp', 'is_udp', 'is_icmp', 'is_http', 'is_ssh',
            
            # Login/Auth features (10)
            'failed_login_count', 'successful_login_count', 'login_failure_rate',
            'unique_usernames', 'unique_passwords', 'password_diversity',
            'username_diversity', 'default_credentials_attempted', 'brute_force_score',
            'credential_stuffing_score',
            
            # Command/Execution features (8)
            'command_count', 'unique_commands', 'dangerous_commands', 'download_attempts',
            'upload_attempts', 'script_execution_score', 'shell_spawning_score',
            'privilege_escalation_score',
            
            # Network behavior (10)
            'bytes_sent', 'bytes_received', 'total_bytes', 'packet_count',
            'avg_packet_size', 'connection_duration', 'connection_count',
            'concurrent_connections', 'connection_failures', 'reconnection_rate',
            
            # HTTP features (8)
            'http_requests', 'http_methods_diversity', 'url_length_avg', 'url_entropy',
            'suspicious_paths', 'sql_injection_indicators', 'xss_indicators',
            'path_traversal_indicators',
            
            # Anomaly indicators (10)
            'entropy_score', 'randomness_score', 'geographic_anomaly', 'temporal_anomaly',
            'behavioral_anomaly', 'reputation_score', 'threat_intelligence_match',
            'known_bad_ip', 'tor_exit_node', 'proxy_detected',
            
            # Statistical features (5)
            'mean_inter_event_time', 'stddev_inter_event_time', 'event_regularity',
            'session_duration', 'activity_variance'
        ]
    ...
    def extract_features(self, src_ip: str, events: List[Event]) -> np.ndarray:
        ...
        if len(sorted_events) > 1 and sorted_events[0].ts and sorted_events[-1].ts:
            ...
            features['time_since_first_event'] = min(time_span / 3600, 1.0)  # Normalize to hours
        else:
        ...
        features['sustained_activity'] = min(time_span / 3600, 1.0) if time_span else 0.0
        ...
        src_ports = [e.src_port for e in events if hasattr(e, 'src_port') and e.src_port]
        ...
        # Login/Auth features
        failed_logins = [e for e in events if 'failed' in e.eventid.lower() or 'fail' in e.message.lower()]
        successful_logins = [e for e in events if 'success' in e.eventid.lower() and 'login' in e.eventid.lower()]
        ...
        features['password_diversity'] = len(passwords) / max(len(usernames), 1) if usernames else 0.0
        features['username_diversity'] = len(usernames) / 50.0
        
        default_creds = {'admin', 'root', 'administrator', 'user', 'guest'}
        features['default_credentials_attempted'] = float(any(u in default_creds for u in usernames))
        ...
        dangerous_keywords = ['rm', 'chmod', 'wget', 'curl', 'nc', 'bash', 'sh', 'python', 'perl', 'eval']
        features['dangerous_commands'] = float(any(
        ...
        features['shell_spawning_score'] = float(any('sh' in e.message.lower() for e in commands))
        features['privilege_escalation_score'] = float(any('sudo' in e.message.lower() or 'su ' in e.message.lower() for e in events))
        ...
        features['url_length_avg'] = sum(len(m) for m in messages) / max(len(messages), 1) / 100.0 if messages else 0.0
        features['url_entropy'] = 0.5
        
        features['suspicious_paths'] = float(any(
            any(pattern in m.lower() for pattern in ['../', 'admin', 'config', 'backup'])
            for m in messages
        ))
        features['sql_injection_indicators'] = float(any(
            any(pattern in m.lower() for pattern in ['or 1=1', 'union select', "'; drop", '--'])
            for m in messages
        ))
        features['xss_indicators'] = float(any(
            any(pattern in m.lower() for pattern in ['<script', 'javascript:', 'onerror='])
            for m in messages
        ))
        ...
        if len(sorted_events) > 1:
            ...
            for i in range(1, len(sorted_events)):
                if sorted_events[i].ts and sorted_events[i-1].ts:
                    delta = (sorted_events[i].ts - sorted_events[i-1].ts).total_seconds()
            ...
            if inter_event_times:
                ...
                features['event_regularity'] = 1.0 - min(np.std(inter_event_times) / max(np.mean(inter_event_times), 1), 1.0)
            else:
