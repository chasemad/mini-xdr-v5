...
class ResponseAnalyticsEngine:
    ...
    async def calculate_comprehensive_metrics(
        ...
        try:
            ...
            comprehensive_metrics = {
                ...
                "effectiveness_analysis": results[1]
    ...
    async def _get_base_analytics_data(
        ...
        if incident_id:
            workflow_query = workflow_query.where(
                ResponseWorkflow.incident_id == incident_id
            )
    ...
    async def _calculate_response_metrics(
        ...
        successful_workflows = len(
            [w for w in workflows if w.success_rate and w.success_rate > 0.8]
        )
        ...
        success_rates = [
            w.success_rate for w in workflows if w.success_rate is not None
        ]
        ...
        total_responses = sum(
            m.attacks_blocked + m.false_positives for m in impact_metrics
        )
        false_positive_rate = (
            false_positives / total_responses if total_responses > 0 else 0.0
        )

        # Mean time to containment (simplified)
        containment_times = [
            w.execution_time_ms for w in workflows if w.status == "completed"
        ]
        mean_time_to_containment = (
            statistics.mean(containment_times) if containment_times else 0.0
        )
        ...
        cost_effectiveness = (
            attacks_blocked / max(total_cost, 1) if total_cost > 0 else attacks_blocked
        )
    ...
    async def _calculate_effectiveness_analysis(
        self, base_data: Dict[str, Any], db_session: AsyncSession
        ...
        for playbook, stats in workflow_stats.items():
            if stats["total"] >= 2:
                avg_success = (
                    statistics.mean(stats["success_rates"])
        ...
        comparative_analysis = await self._calculate_comparative_analysis(
            workflows, db_session
        )
    ...
    async def _calculate_trend_analysis(
        ...
        while current_time < end_time:
            ...
            if bucket_workflows:
                success_rates = [
                    w.success_rate for w in bucket_workflows if w.success_rate
                ]
                response_times = [
                    w.execution_time_ms for w in bucket_workflows if w.execution_time_ms
                ]
                ...
                trend_point = TrendData(
                    ...
                    effectiveness_score=self._calculate_bucket_effectiveness(
                        bucket_workflows
                    ),
                )
        ...
        if len(trend_buckets) >= 2:
            success_trend = self._calculate_linear_trend(
                [t.success_rate for t in trend_buckets]
            )
            response_time_trend = self._calculate_linear_trend(
                [t.response_time for t in trend_buckets]
            )
            volume_trend = self._calculate_linear_trend(
                [float(t.incident_volume) for t in trend_buckets]
            )
        else:
    ...
    async def _calculate_business_impact_metrics(
        self, base_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        roi = (
            (estimated_attack_cost - total_cost_impact) / max(total_cost_impact, 1)
        ...
        return {
            ...
            "business_impact_trends": await self._calculate_business_impact_trends(
                base_data
            ),
        }
    ...
    async def _calculate_compliance_metrics(
        ...
        gdpr_compliant = len(
            [
                w
                for w in workflows
                if w.execution_time_ms and w.execution_time_ms <= 72 * 3600 * 1000
            ]
        )  # 72 hours
        sox_compliant = len(
            [
                w
                for w in workflows
                if w.execution_time_ms and w.execution_time_ms <= 24 * 3600 * 1000
            ]
        )  # 24 hours
        ...
        return {
            ...
            "gdpr_compliance_rate": gdpr_compliant / total_workflows
            if total_workflows > 0
            else 0,
            "sox_compliance_rate": sox_compliant / total_workflows
            if total_workflows > 0
            else 0,
            "regulatory_metrics": {
    ...
    def _assess_data_quality(self, base_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Data completeness
        complete_workflows = len(
            [w for w in workflows if w.success_rate is not None and w.execution_time_ms]
        )
        ...
        recent_workflows = len(
            [
                ...
                for w in workflows
                if w.created_at
                and w.created_at > datetime.now(timezone.utc) - timedelta(days=7)
            ]
        )
        ...
        return {
            ...
            "completeness_percentage": (complete_workflows / len(workflows)) * 100
            if workflows
            else 0,
            "data_span_days": (
                max(w.created_at for w in workflows if w.created_at)
                - min(w.created_at for w in workflows if w.created_at)
            ).days
            if workflows
            else 0,
        }

    async def _calculate_effectiveness_trends(
        ...
        for i in range(window_size, len(sorted_workflows) + 1):
            ...
            success_rates = [
                w.success_rate for w in window if w.success_rate is not None
            ]
            if success_rates:
    ...
    def _generate_improvement_recommendations(
        ...
        if action_effectiveness:
            best_actions = sorted(
                action_effectiveness.items(), key=lambda x: x[1], reverse=True
            )
            worst_actions = sorted(action_effectiveness.items(), key=lambda x: x[1])

            if best_actions and best_actions[0][1] > 0.9:
                recommendations.append(
                    f"Consider using {best_actions[0][0]} more frequently (98% success rate)"
                )

            if worst_actions and worst_actions[0][1] < 0.5:
                recommendations.append(
                    f"Review {worst_actions[0][0]} implementation (low success rate)"
                )

        # Workflow-based recommendations
        if workflow_effectiveness:
            best_workflows = sorted(
                workflow_effectiveness.items(), key=lambda x: x[1], reverse=True
            )

            if best_workflows and best_workflows[0][1] > 0.9:
                recommendations.append(
                    f"Use '{best_workflows[0][0]}' as template for similar incidents"
                )
    ...
    async def _calculate_comparative_analysis(
        self, workflows: List[ResponseWorkflow], db_session: AsyncSession
    ) -> Dict[str, Any]:
        ...
        our_success_rate = statistics.mean(
            [w.success_rate for w in workflows if w.success_rate]
        )
        our_response_time = statistics.mean(
            [w.execution_time_ms for w in workflows if w.execution_time_ms]
        )
        ...
        success_comparison = (
            our_success_rate / industry_benchmarks["success_rate"] - 1
        ) * 100
        time_comparison = (
            1 - our_response_time / industry_benchmarks["response_time_ms"]
        ) * 100
        ...
        return {
            ...
            "competitive_position": self._assess_competitive_position(
                success_comparison, time_comparison
            ),
            "benchmark_sources": [
    ...
    async def _identify_optimization_opportunities(
        ...
        slow_workflows = [
            w
            for w in workflows
            if w.execution_time_ms and w.execution_time_ms > 1800000
        ]  # > 30 min
        if slow_workflows:
    ...
    def _generate_executive_summary(
        self, analytics_results: List[Any]
    ) -> Dict[str, Any]:
        ...
        try:
            response_metrics = (
                analytics_results[0]
                if len(analytics_results) > 0
                and not isinstance(analytics_results[0], Exception)
                else None
            )
            effectiveness = (
                analytics_results[1]
                if len(analytics_results) > 1
                and not isinstance(analytics_results[1], Exception)
                else None
            )
            trends = (
                analytics_results[2]
                if len(analytics_results) > 2
                and not isinstance(analytics_results[2], Exception)
                else None
            )
            ...
            return {
                ...
                "recommendation": self._get_executive_recommendation(
                    overall_score, key_insights
                ),
            }
    ...
    async def _generate_analytics_recommendations(
        self, analytics_results: List[Any], base_data: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        ...
        try:
            response_metrics = (
                analytics_results[0] if len(analytics_results) > 0 else None
            )
    ...
    # Helper methods
    def _calculate_bucket_effectiveness(
        self, workflows: List[ResponseWorkflow]
    ) -> float:
    ...
    def _assess_trend_quality(self, trend_buckets: List[TrendData]) -> Dict[str, Any]:
        ...
        return {
            ...
            "quality": "high"
            if len(trend_buckets) >= 20
            else "medium"
            if len(trend_buckets) >= 10
            else "low",
    ...
    def _generate_trend_predictions(
        self, trend_buckets: List[TrendData]
    ) -> Dict[str, Any]:
    ...
    async def _calculate_business_impact_trends(
        self, base_data: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _calculate_audit_readiness(
        ...
        # Compliance tracking
        compliance_tracked = len(
            [m for m in impact_metrics if m.compliance_impact != "none"]
        )
        if impact_metrics:
            readiness_score += (compliance_tracked / len(impact_metrics)) * 0.3

        # Response time compliance
        compliant_responses = len(
            [
                w
                for w in workflows
                if w.execution_time_ms and w.execution_time_ms <= 86400000
            ]
        )  # 24h
        if workflows:
    ...
    async def _calculate_compliance_trends(
        ...
        for i in range(window_size, len(sorted_metrics) + 1):
            ...
            violations = len(
                [m for m in window if m.compliance_impact in ["high", "critical"]]
            )
    ...
    def _assess_competitive_position(
        self, success_comparison: float, time_comparison: float
    ) -> str:
    ...
    def _calculate_impact_distribution(
        self, metrics: List[ResponseImpactMetrics]
    ) -> Dict[str, Any]:
    ...
    def _get_executive_recommendation(
        self, overall_score: float, insights: List[str]
    ) -> str:
        ...
        if overall_score > 0.8:
            ...
            return (
                "Focus on improving response effectiveness and reducing false positives"
            )
        else:
