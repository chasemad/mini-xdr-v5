...
class BehaviorAnalyzer:
    ...
    async def analyze_ip_behavior(self, db: AsyncSession, src_ip: str, window_minutes: int = 30) -> Optional[BehavioralPattern]:
        ...
        if threat_score > self.adaptive_threshold:
            return self._create_behavioral_incident(src_ip, anomaly_scores, features, threat_score)
            
        return None
    
    async def _get_recent_events(self, db: AsyncSession, src_ip: str, window_minutes: int) -> List[Event]:
    ...
    async def _extract_behavioral_features(self, events: List[Event]) -> Dict[str, float]:
        ...
        # Time-based features
        time_span = (events[0].ts - events[-1].ts).total_seconds() if len(events) > 1 else 1
        ...
        if len(events) > 1:
            ...
            features['request_interval_variance'] = np.var(intervals) if intervals else 0
            features['sub_second_requests'] = sum(1 for i in intervals if i < 1)
            features['perfectly_timed_requests'] = sum(1 for i in intervals if abs(i - round(i)) < 0.1)
        ...
        web_events = [e for e in events if 'http' in str(e.raw).lower() or e.dst_port in [80, 443, 8080]]
        if web_events:
    ...
    async def _extract_web_features(self, web_events: List[Event]) -> Dict[str, float]:
        ...
        features['sql_injection_attempts'] = sum(1 for indicator in attack_indicators if 'sql' in indicator.lower())
        features['path_traversal_attempts'] = sum(1 for indicator in attack_indicators if 'traversal' in indicator.lower())
        features['injection_patterns'] = len([i for i in attack_indicators if any(pattern in i.lower() for pattern in ['injection', 'script', 'payload'])])
        
        # Sophistication analysis
        features['attack_sophistication'] = self._calculate_attack_sophistication(attack_indicators)
        features['encoding_attempts'] = sum(1 for indicator in attack_indicators if 'encoding' in indicator.lower())
    ...
    async def _extract_ssh_features(self, ssh_events: List[Event]) -> Dict[str, float]:
    ...
    def _calculate_attack_sophistication(self, attack_indicators: List[str]) -> float:
        ...
        for indicator in attack_indicators:
            ...
            elif any(intermediate in indicator_lower for intermediate in ['bypass', 'evasion', 'encoding']):
                sophistication_score += 0.3
            # Advanced attacks
            elif any(advanced in indicator_lower for advanced in ['obfuscation', 'polyglot', 'advanced']):
    ...
    def _create_behavioral_incident(self, src_ip: str, anomaly_scores: Dict[str, float], 
                                  features: Dict[str, float], threat_score: float) -> BehavioralPattern:
        ...
        # Create description
        active_patterns = [name for name, score in anomaly_scores.items() if score > 0.3]
