...
try:
    ...
    from .federated_learning import FederatedModelType, FederatedRole, federated_manager
...
class LSTMAutoencoder(nn.Module):
    ...
    def __init__(
        ...
        self.decoder = nn.LSTM(
            hidden_size, input_size, num_layers, batch_first=True, dropout=0.2
        )
...
class IsolationForestDetector(BaseMLDetector):
    ...
    async def train_model(self, training_data: List[Dict[str, float]]) -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"Isolation Forest trained with {len(training_data)} samples"
            )
            return True
...
class LSTMDetector(BaseMLDetector):
    ...
    async def train_model(self, training_data: List[Dict[str, float]]) -> bool:
        ...
        try:
            ...
            self.is_trained = True
            self.logger.info(
                f"LSTM autoencoder trained with threshold {self.reconstruction_threshold:.4f}"
            )
            return True
    ...
    def _prepare_training_sequences(
        ...
        for i in range(len(df) - self.sequence_length + 1):
            sequence = (
                df.iloc[i : i + self.sequence_length][self.feature_columns]
                .fillna(0)
                .values
            )
    ...
    async def analyze_events(self, events: List[Event]) -> float:
        ...
        try:
            ...
            if "isolation_forest" in self.models:
                try:
                    iso_score = self.models["isolation_forest"].decision_function(
                        features_scaled
                    )[0]
            ...
            if "one_class_svm" in self.models:
                try:
                    svm_prediction = self.models["one_class_svm"].predict(
                        features_scaled
                    )[0]
                    svm_score = self.models["one_class_svm"].decision_function(
                        features_scaled
                    )[0]
                    # Convert to 0-1 scale
                    scores["one_class_svm"] = (
                        1.0 if svm_prediction == -1 else max(0, -svm_score / 2 + 0.5)
                    )
            ...
            if "local_outlier_factor" in self.models:
                try:
                    lof_score = self.models["local_outlier_factor"].decision_function(
                        features_scaled
                    )[0]
                    # Convert to 0-1 scale
            ...
            if scores and total_weight > 0:
                weighted_sum = sum(
                    score * self.weights[model] for model, score in scores.items()
                )
    ...
    async def train_ensemble(self, training_data: List[Dict[str, float]]) -> bool:
        ...
        try:
            if len(training_data) < 50:
                self.logger.warning(
                    "Insufficient training data for enhanced ML ensemble"
                )
                return False
            ...
            for data_point in training_data:
                feature_vector = [
                    data_point.get(col, 0.0) for col in detector.feature_columns
                ]
            ...
            if successful_models > 0:
                self.is_trained = True
                self.logger.info(
                    f"Enhanced ML ensemble trained with {successful_models} models"
                )
                return True
...
class EnsembleMLDetector:
    ...
    async def train_models(
        ...
        results["isolation_forest"] = await self.isolation_forest.train_model(
            training_data
        )
        results["lstm"] = await self.lstm_detector.train_model(training_data)

        # Train enhanced ML ensemble
        results["enhanced_ml"] = await self.enhanced_detector.train_ensemble(
            training_data
        )
...
class FederatedEnsembleDetector:
    ...
    def __init__(self):
        ...
        self.federated_config = {
            "min_participants": 2,
            "aggregation_protocol": AggregationProtocol.SIMPLE_ENCRYPTION
    ...
    def _initialize_federated_components(self):
        ...
        try:
            if FEDERATED_AVAILABLE:
                ...
                self.logger.info(
                    "Federated learning components initialized successfully"
                )
        except Exception as e:
    ...
    async def calculate_anomaly_score(self, src_ip: str, events: List[Event]) -> float:
        ...
        try:
            ...
            ensemble_score = await self.ensemble_detector.calculate_anomaly_score(
                src_ip, events
            )
    ...
    async def train_models(
        self, training_data: List[Dict[str, float]], enable_federated: bool = True
    ) -> Dict[str, bool]:
            ...
            status.update(
                {
                    ...
                    "last_federated_training": self.last_federated_training.isoformat()
                    if self.last_federated_training
                    else None,
...
class EnhancedFederatedDetector:
    ...
    def __init__(self):
        ...
        self.deep_learning_manager = (
            deep_learning_manager if DEEP_LEARNING_AVAILABLE else None
        )
    ...
    async def calculate_anomaly_score(self, src_ip: str, events: List[Event]) -> float:
        ...
        try:
            ...
            try:
                from local_inference import (  # Optional local inference helper
                    local_ml_client,
                )
            except ImportError:
                self.logger.debug(
                    "local_inference module not available; skipping dedicated local client"
                )
            except Exception as e:
                self.logger.debug(f"Local inference import failed: {e}")
            else:
                try:
                    if await local_ml_client.health_check():
                        ...
                        feature_vector = ml_feature_extractor.extract_features(
                            src_ip, events
                        )
                        ...
                        ml_events = [
                            {
                                ...
                                "timestamp": events[0].ts.isoformat()
                                if events and events[0].ts
                                else None,
                                "raw": events[0].raw
                                if events and hasattr(events[0], "raw")
                                else {},
                        ...
                        if results:
                            ...
                            predicted_class = results[0].get(
                                "predicted_class", "Unknown"
                            )
                            confidence = results[0].get("confidence", 0.0)

                            self.logger.info(
                                f"Local ML: {predicted_class} (conf: {confidence:.3f}, score: {local_ml_score:.3f})"
                            )
                except Exception as e:
            ...
            if self.deep_learning_enabled and self.deep_learning_manager:
                try:
                    deep_result = (
                        await self.deep_learning_manager.calculate_threat_score(
                            src_ip, events
                        )
                    )
            ...
            if local_ml_score > 0:
                ...
                self.logger.info(
                    f"Local ML scoring - ML: {local_ml_score:.3f}, "
                ...
                self.logger.debug(
                    f"Deep learning - Deep: {deep_score:.3f}, "
                    f"Traditional: {traditional_score:.3f}, Combined: {combined_score:.3f}"
                )
            else:
    ...
    async def train_models(
        ...
        results = await self.federated_detector.train_models(
            training_data, enable_federated
        )

        # Note: Deep learning models are trained locally
    ...
    def load_deep_learning_models_from_local(self, model_path: str) -> bool:
        """Load deep learning models from local path"""
        ...
        try:
            ...
            results = self.deep_learning_manager.load_models(model_path)
            self.deep_learning_enabled = any(results.values())

            if self.deep_learning_enabled:
                self.logger.info(
                    f"Successfully loaded deep learning models from {s3_model_path}"
                )
                return True
            else:
