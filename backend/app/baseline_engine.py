...
class BaselineEngine:
    ...
    async def update_baselines(self, db: AsyncSession):
        ...
        try:
            ...
            logger.warning(
                f"Baseline update failed (this is expected with empty database): {e}"
            )

    async def _get_clean_data_period(
        ...
        clean_days = sorted(daily_incidents.items(), key=lambda x: x[1])[
            : max(1, days // 2)
        ]
        ...
        for clean_date in clean_day_dates:
            ...
            query = (
                select(Event)
                .where(and_(Event.ts >= day_start, Event.ts < day_end))
                .order_by(Event.ts)
            )
        ...
        clean_events = {
            ip: events
            for ip, events in clean_events.items()
            if ip not in incident_ips and len(events) >= self.min_samples_for_baseline
        }

        logger.info(
            f"Found clean data for {len(clean_events)} IPs from {len(clean_day_dates)} clean days"
        )
        return clean_events

    async def _get_daily_incident_counts(
        ...
        try:
            ...
            logger.warning(f"Failed to get daily incident counts: {e}")
            return {}

    async def _get_incident_ips_in_period(
        self, db: AsyncSession, start_date: datetime
    ) -> set:
        ...
        try:
            ...
            logger.warning(f"Failed to get incident IPs: {e}")
            return set()
    ...
    async def _learn_parameter_patterns(self, db: AsyncSession):
        ...
        if param_metrics:
            self.baselines["global"]["normal_param_diversity_mean"] = np.mean(
                [m["param_diversity"] for m in param_metrics]
            )
            self.baselines["global"]["normal_param_diversity_std"] = np.std(
                [m["param_diversity"] for m in param_metrics]
            )
            self.baselines["global"]["normal_unique_params_mean"] = np.mean(
                [m["unique_params"] for m in param_metrics]
            )
            self.baselines["global"]["normal_unique_params_std"] = np.std(
                [m["unique_params"] for m in param_metrics]
            )
    ...
    async def _learn_global_patterns(self, db: AsyncSession):
        ...
        self.baselines["global"][
            "avg_global_requests_per_hour"
        ...
        for metric, current_value in current_metrics.items():
            if metric in baseline_metrics:
                ...
                if std_key in baseline_metrics and baseline_metrics[std_key] > 0:
                    z_score = (
                        abs(current_value - baseline_value) / baseline_metrics[std_key]
                    )
    ...
    def get_baseline_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "baseline_sample": {
                "sample_ip_baseline": list(self.baselines["per_ip"].keys())[:3]
                if self.baselines["per_ip"]
                else [],
    ...
    def adjust_sensitivity(self, sensitivity_level: str):
        ...
        if sensitivity_level == "high":
            self.deviation_thresholds = {"minor": 1.5, "moderate": 2.0, "severe": 2.5}
            self.baseline_confidence_threshold = 0.6
        elif sensitivity_level == "low":
            self.deviation_thresholds = {"minor": 3.0, "moderate": 4.0, "severe": 5.0}
            self.baseline_confidence_threshold = 0.8
        else:  # medium (default)
            self.deviation_thresholds = {"minor": 2.0, "moderate": 3.0, "severe": 4.0}
            self.baseline_confidence_threshold = 0.7
