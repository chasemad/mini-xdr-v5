...
class FederatedNode(Base):
    ...
    role = Column(String(16), default=FederatedRoleEnum.PARTICIPANT)  # coordinator|participant|observer
    ...
    compute_capacity = Column(Float, default=1.0)  # Relative compute capacity (0.1 - 10.0)
    ...
    average_training_time = Column(Float, nullable=True)  # Average training time in seconds
    ...
    # Relationships
    training_rounds = relationship("FederatedTrainingRound", back_populates="participants", secondary="federated_round_participants")
...
class FederatedTrainingRound(Base):
    ...
    # Model information
    model_type = Column(String(32), index=True)  # isolation_forest, lstm_autoencoder, etc.
    ...
    aggregation_method = Column(String(32), default="federated_averaging")  # federated_averaging, median, etc.
    ...
    model_weights_hash = Column(String(64), nullable=True)  # Hash of final aggregated weights
    ...
    # Relationships
    participants = relationship("FederatedNode", back_populates="training_rounds", secondary="federated_round_participants")
    model_updates = relationship("FederatedModelUpdate", back_populates="training_round")
...
class FederatedRoundParticipants(Base):
    ...
    round_id = Column(Integer, ForeignKey("federated_training_rounds.id"), primary_key=True)
    node_id = Column(Integer, ForeignKey("federated_nodes.id"), primary_key=True)
    
    # Participation metadata
    joined_at = Column(DateTime(timezone=True), server_default=func.now())
    status = Column(String(16), default="invited")  # invited|training|completed|failed|dropped
...
class FederatedModelUpdate(Base):
    ...
    # Relationships
    training_round = relationship("FederatedTrainingRound", back_populates="model_updates")
...
class FederatedModelRegistry(Base):
    ...
    status = Column(String(16), default="training")  # training|active|deprecated|archived
...
class FederatedAuditLog(Base):
    ...
    # Event information
    event_type = Column(String(32), index=True)  # node_join, round_start, model_update, etc.
    node_id = Column(String(64), index=True)
...
class FederatedPerformanceMetrics(Base):
    ...
    # Reference
    round_id = Column(Integer, ForeignKey("federated_training_rounds.id"), nullable=True, index=True)
    node_id = Column(String(64), index=True)
    ...
    # Efficiency scores
    communication_efficiency = Column(Float, nullable=True)  # Data transferred per accuracy gain
...
class FederatedConfiguration(Base):
    ...
    target_id = Column(String(64), nullable=True, index=True)  # Target node/model ID if applicable
...
FEDERATED_MODEL_SCHEMAS = {
    ...
    "lstm_autoencoder": {
        ...
        "optional_params": ["num_layers", "dropout", "learning_rate", "batch_size", "epochs"],
        "param_types": {
            "input_size": int,
            "hidden_size": int,
            "sequence_length": int,
    ...
    "neural_network": {
        ...
        "optional_params": ["activation", "optimizer", "learning_rate", "batch_size", "epochs"],
        "param_types": {
            "input_size": int,
            "hidden_layers": list,
            "output_size": int,
...
def validate_federated_model_config(model_type: str, config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for param, value in config.items():
        if param in schema["param_types"]:
            ...
            if not isinstance(value, expected_type):
                errors.append(f"Parameter {param} should be of type {expected_type}, got {type(value)}")
