...
from ..models import AgentEnrollment, CloudAsset
...
class SmartDeploymentEngine:
    ...
    def __init__(self, organization_id: int, db: AsyncSession):
        ...
        self.enrollment_service = AgentEnrollmentService(organization_id, db)
    ...
    async def deploy_to_assets(
        self, assets: List[Dict[str, Any]], provider: str
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            deployable_assets = [
                a
                for a in assets
                if a.get("agent_compatible", False)
                and not a.get("agent_deployed", False)
            ]

            # Sort by priority: critical > high > medium > low
            priority_order = {"critical": 0, "high": 1, "medium": 2, "low": 3}
            deployable_assets.sort(
            ...
            logger.info(
                f"Smart deployment completed: {deployment_results['success']} "
                f"succeeded, {deployment_results['failed']} failed"
            )
    ...
    async def get_deployment_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "deployment_in_progress": sum(
                1 for a in assets if a.agent_status == "deploying"
            ),
