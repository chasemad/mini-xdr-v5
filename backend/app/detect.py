...
class SlidingWindowDetector:
    ...
    async def evaluate(self, db: AsyncSession, src_ip: str) -> Optional[Dict[str, Any]]:
        ...
        if existing_incident:
            logger.debug(
                f"Open incident already exists for {src_ip}: {existing_incident.id}"
            )
            return None
        ...
        logger.info(
            f"SSH brute-force check: {src_ip} has {fail_count} failures in last {self.window_seconds}s (threshold: {self.threshold})"
        )

        if fail_count >= self.threshold:
            # Calculate dynamic risk score based on intensity
            # Base score 0.6, +0.01 per failure, capped at 0.95
            risk_score = min(0.6 + (fail_count / 100.0), 0.95)
            severity = "high" if fail_count > (self.threshold * 2) else "medium"
            ...
            incident_data = {
                ...
                "containment_confidence": 0.9,  # High confidence for rule-based detection
            ...
            logger.warning(
                f"NEW INCIDENT #{incident.id}: SSH brute-force from {src_ip}"
            )
    ...
    async def _get_open_incident(
        ...
        query = (
            select(Incident)
            .where(and_(Incident.src_ip == src_ip, Incident.status == "open"))
...
class AdvancedCorrelationEngine:
    ...
    async def analyze_ip(self, db: AsyncSession, src_ip: str) -> List[AttackPattern]:
        ...
        events_medium = await self._get_failed_login_events(
            db, src_ip, self.medium_window
        )
    ...
    async def _get_failed_login_events(
        self, db: AsyncSession, src_ip: str, window_seconds: int
    ) -> List[Event]:
    ...
    async def _detect_password_spray(
        ...
        total_unique_passwords = (
            len(set().union(*username_passwords.values())) if username_passwords else 0
        )
        ...
        for username, passwords in username_passwords.items():
            if (
                len(passwords)
                >= self.thresholds["password_spray"]["min_unique_passwords"]
            ):
        ...
        if (
            total_unique_passwords
            >= self.thresholds["password_spray"]["min_unique_passwords"]
            and total_usernames <= 5
            and spray_indicators  # Few usernames
        ):
            ...
            return AttackPattern(
                ...
                time_span=int((events[0].ts - events[-1].ts).total_seconds())
                if len(events) > 1
                else 0,
...
class WebAttackDetector:
    ...
    async def evaluate(self, db: AsyncSession, src_ip: str) -> Optional[Dict[str, Any]]:
        ...
        logger.info(
            f"Web attack check: {src_ip} has {total_indicators} attack indicators in last {self.window_seconds}s (threshold: {self.threshold})"
        )
        ...
        if total_indicators >= self.threshold:
            ...
            attack_description = (
                ", ".join(attack_types) if attack_types else "Web application attacks"
            )
            ...
            incident_data = {
                ...
                "status": "open",
                "auto_contained": False,
                "risk_score": min(0.7 + (total_indicators / 10.0), 0.98),
                "escalation_level": severity,
                "threat_category": "web_attack",
                "containment_confidence": 0.85,
                "containment_method": "rule_based",
                "ml_features": {"indicators": total_indicators, "types": attack_types},
            }
            ...
            logger.warning(
                f"NEW WEB INCIDENT #{incident.id}: {attack_description} from {src_ip}"
            )
    ...
    def _infer_attack_indicators(
        self, event: Event, raw_data: Dict[str, Any]
    ) -> List[str]:
        ...
        if raw_data.get("filename") and str(raw_data.get("filename")).lower().endswith(
            (".php", ".jsp", ".asp", ".exe", ".sh")
        ):
...
from .threat_detectors import SpecializedDetectionResult, specialized_detector
...
class AdaptiveDetectionEngine:
    ...
    async def comprehensive_analysis(
        ...
        if ddos_detection:
            ddos_detection.indicators[
                "mitigation_actions"
            ] = self.specialized_detector.get_ddos_actions(recent_events)
        ...
        suricata_incident = await self._correlate_suricata_alerts(
            db, src_ip, recent_events
        )
        ...
        if detection_results:
            composite_incident = await self._create_composite_incident(
                db, src_ip, detection_results
            )
    ...
    async def _get_recent_events(
        self, db: AsyncSession, src_ip: str, minutes: int
    ) -> List[Event]:
    ...
    async def _create_composite_incident(
        self, db: AsyncSession, src_ip: str, detection_results: Dict
    ) -> Dict:
        ...
        if "behavioral" in detection_results:
            behavior_desc = detection_results["behavioral"].get(
        ...
        for event in recent_events:
            ...
            if any(keyword in event_text for keyword in critical_keywords):
                ...
                logger.info(
                    f"Critical indicator detected in adaptive detection: {event.eventid}"
                )
                break
        ...
        if has_critical_indicators and severity not in ["critical", "high"]:
            ...
            logger.info(
                f"Elevated adaptive detection to HIGH priority due to critical threat indicators for {src_ip}"
            )
    ...
    async def _create_special_incident(
        ...
        if existing_incident:
            logger.debug(
                "Special detection skipped: incident already open for %s", src_ip
            )
            return {"incident_id": existing_incident.id}

        severity_order = {"critical": 4, "high": 3, "medium": 2, "low": 1}
        primary_detection = max(
            detections, key=lambda d: (severity_order.get(d.severity, 0), d.confidence)
        )
    ...
    def _detect_ddos(self, events: List[Event]) -> Optional[SpecializedDetectionResult]:
        ...
        concentration_ratio = (
            most_common_count / len(sorted_events) if sorted_events else 0.0
        )
    ...
    async def _correlate_suricata_alerts(
        self, db: AsyncSession, src_ip: str, recent_events: List[Event]
    ...
    try:
        ...
        if patterns:
            ...
            highest_severity_pattern = max(
                patterns, key=lambda p: {"low": 1, "medium": 2, "high": 3}[p.severity]
            )
            incident_id = await _create_correlation_incident(
                db, highest_severity_pattern, patterns
            )
            return incident_id
...
async def _create_correlation_incident(
    db: AsyncSession, primary_pattern: AttackPattern, all_patterns: List[AttackPattern]
) -> int:
    """Create an incident based on correlation analysis"""

    # Check if there's already an open incident for this IP
    existing_incident = await ssh_bruteforce_detector._get_open_incident(
        db, primary_pattern.src_ip
    )
    if existing_incident:
        logger.debug(
            f"Open incident already exists for {primary_pattern.src_ip}: {existing_incident.id}"
        )
    ...
    incident_data = {
        ...
        "status": "open",
        "auto_contained": False,
        "risk_score": primary_pattern.confidence,
        "escalation_level": primary_pattern.severity,
        "threat_category": primary_pattern.pattern_type,
        "containment_confidence": primary_pattern.confidence,
        "containment_method": "correlation_engine",
        "ml_features": {"pattern_count": len(all_patterns)},
    }
    ...
    logger.warning(
        f"NEW CORRELATION INCIDENT #{incident.id}: {primary_pattern.pattern_type} from {primary_pattern.src_ip}"
    )
