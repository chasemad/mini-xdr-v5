    ...
    if read -r command < "$FIFO"; then
        ...
        case "$command" in
            "block:"*)
                ...
                ssh -p 22022 -i ~/.ssh/xdrops_id_ed25519 xdrops@192.168.168.133 "sudo ufw deny from $IP to any" >> "$LOG_FILE" 2>&1
                echo "Block result: $?" >> "$LOG_FILE"
                ;;
            "unblock:"*)
                IP="${command#unblock:}"
                echo "Unblocking IP: $IP" >> "$LOG_FILE"
                ssh -p 22022 -i ~/.ssh/xdrops_id_ed25519 xdrops@192.168.168.133 "sudo ufw delete deny from $IP to any" >> "$LOG_FILE" 2>&1
                echo "Unblock result: $?" >> "$LOG_FILE"
                ;;
            "status")
                echo "Getting UFW status" >> "$LOG_FILE"
                ssh -p 22022 -i ~/.ssh/xdrops_id_ed25519 xdrops@192.168.168.133 "sudo ufw status numbered" >> "$LOG_FILE" 2>&1
                ;;
        esac
    fi
done
