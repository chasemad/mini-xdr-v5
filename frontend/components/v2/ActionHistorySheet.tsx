...
interface ActionHistoryProps {
  isOpen: boolean;
  onClose: () => void;
  actions: any[]; // Using any for now to match mixed types (agent vs playbook)
...
export default function ActionHistorySheet({ isOpen, onClose, actions, onRollback }: ActionHistoryProps) {
  ...
  return (
    <Sheet open={isOpen} onOpenChange={onClose}>
      <SheetContent className="w-[800px] sm:max-w-[800px] p-0 flex flex-col bg-background border-l">
              ...
              <div className="flex-1 flex flex-col overflow-hidden">
                 <div className="p-6 pb-4 border-b bg-card shrink-0">
                    <div className="flex items-center gap-3 mb-4">
                       <div className={cn("p-2 rounded-lg border", getStatusColor(selectedAction.status))}>
                          {getStatusIcon(selectedAction.status)}
                       </div>
                       <div>
                          <h2 className="text-lg font-bold leading-tight">
                             {selectedAction.action || selectedAction.action_name}
                          </h2>
                          <div className="flex items-center gap-2 mt-1 text-xs text-muted-foreground">
                             <span className="font-mono">{selectedAction.id || "ID: N/A"}</span>
                             <span>•</span>
                             <span>{new Date(selectedAction.timestamp || selectedAction.created_at).toLocaleString()}</span>
                          </div>
                       </div>
                    </div>
                    ...
                    <div className="p-6 space-y-6">
                       ...
                       {(selectedAction.parameters || selectedAction.params) && (
                          <div className="space-y-2">
                             ...
                             <div className="relative group">
                                <pre className="p-3 rounded-md bg-zinc-950 text-zinc-50 text-xs overflow-x-auto font-mono border border-zinc-800">
                       ...
                       {selectedAction.result_data && (
                          <div className="space-y-2">
                             ...
                             <pre className="p-3 rounded-md bg-green-950/30 text-green-400 text-xs overflow-x-auto font-mono border border-green-900/50">
                       ...
                       {selectedAction.error_details && (
                          <div className="space-y-2">
                             ...
                             <pre className="p-3 rounded-md bg-red-950/30 text-red-400 text-xs overflow-x-auto font-mono border border-red-900/50">
